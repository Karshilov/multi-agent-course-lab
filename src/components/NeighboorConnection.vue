...
const generateEdgeData = (value: number, kLimit: number) => {
  const edges = [];
  for (let i = 0; i < value; i++) {
    for (let j = i + 1; j < value; j++) {
      if (Math.min(j - i, value - j + i) <= kLimit / 2) {
        edges.push({
          source: `node${i}`,
          target: `node${j}`,
          type: "quadratic",
...
const drawGraph = (value: number, kLimit: number) => {
  if (graph) {
    nodes.value = generateNodeData(value, graph.getWidth(), graph.getHeight());
    edges.value = generateEdgeData(value, kLimit);
    const data = {
      nodes: nodes.value,
      edges: edges.value,
    };
...
onMounted(() => {
  graph = new G6.Graph({
    container: "neighboor-connection",
    width: 800,
    height: 500,
    modes: {
      default: ['drag-node'],
...
<template>
  <n-space vertical>
    <n-slider :min="2" :max="nodeCount" :step="2" v-model:value="kNearest"
      v-on:update:value="drawGraph(nodeCount, kNearest)">
    </n-slider>
    <n-slider :min="2" :max="15" v-model:value="nodeCount" v-on:update:value="drawGraph(nodeCount, kNearest)">
    </n-slider>
    <div id="neighboor-connection"></div>
